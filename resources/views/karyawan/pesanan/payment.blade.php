...
<div class="p-5 rounded-lg shadow-md bg-white">
    <div class="flex justify-between items-center">
        <u class="font-extrabold text-3xl">karyawan | Pembayaran Pesanan</u>
    </div>
</div>
            ...
            payButton.addEventListener('click', function() {
                snap.pay(snapToken, {
                    onSuccess: function(result) {
                        window.location.href = "{{ route('karyawan.pesanans.index') }}?success=true&order_id={{ $pesanan->id_pesanan }}";
                    },
                    onPending: function(result) {
                        window.location.href = "{{ route('karyawan.pesanans.index') }}?pending=true&order_id={{ $pesanan->id_pesanan }}";
                    },
